---
description: "Fetch and triage PR review comments and CI failures for the current branch. Presents findings for approval before applying fixes."
agent: "pr-reviewer"
...
3. **Coverage check**: Assess patch coverage from Codecov:
   ...
   - The PR comment only shows the top 10 files. To get the **full file list**, fetch the Codecov tree view: `https://app.codecov.io/gh/oakwood-commons/scafctl-plugin-github/pull/<PR_NUMBER>?dropdown=coverage&src=pr&el=tree`
...
5. **Triage**: For each unresolved comment, pipeline failure, and coverage gap, assess whether it's a legit problem with the code. Present the triage summary with recommendations and **wait for user approval** before making any changes

Include thread IDs in the triage output so fixes can be tracked back to review threads.
